...
async function addListenerToEditBtns() {
    ...
    for ( const btn of editBtns ) {
        btn.addEventListener( "click", function () {
            ...
            document.getElementById( "company_id__input" ).value = rowElems[ 0 ].textContent;
            document.getElementById( "company_title" ).value = rowElems[ 1 ].textContent;
            document.getElementById( "company_position" ).value = rowElems[ 2 ].textContent;
        } );
    }
}
    ...
    document.getElementById( "sort__select" ).addEventListener( "change", function ( event ) {
        ...
        axios.get( `${ location.href }/sort?sort=${ sortType }` )
            .then( function ( responce ) {
                ...
                document.getElementById( "company_table__section" ).innerHTML = pageEl;
                addListenerToEditBtns();
                addListenerToDeleteBtns();
            } )
            .catch( err => console.error( err.message ) );
...
const clearFormInModal = function () {
    document.getElementById( "company_id__input" ).value = "";
